...
downloadmissedmms() {
	...
	count="$(wc -l < cat "$SERVER_MMS_TEMP")"
	echo "Got $count messages."
	if [ "$count" -gt 0 ]; then
		read -rp "Warning. This will download all the undownloaded MMS messages still on server and process them. Continue?  Any key to continue."
